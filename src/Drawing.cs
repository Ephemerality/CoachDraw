    ...
    public class drawObj
    {
        ...
        public void draw(Graphics g, bool selected)
        {
            ...
            switch (objType)
            {
                ...
                case (ItemType.Pylon):
                    g.FillPolygon(new SolidBrush(color), new Point[] { new Point(objLoc.X - 6, objLoc.Y + 6), new Point(objLoc.X + 6, objLoc.Y + 6), new Point(objLoc.X, objLoc.Y - 6) });
